...
def get_courses(bs_tag):
    ...
    whole = []
    for tr in bs_tag.find_all('tr')[1:]:
        # 每门课程的相关信息：课程编号、课程名称、课程学时、课程学分、上课教师、选课类型、上课时间
        tds = tr.find_all('td')
        # td1：课程名称
        td1 = ' '.join(tds[1].get_text().split())
        # td4：上课老师
        td4 = ' '.join(re.findall(r'[\u4e00-\u9fa5]{2,3}',tds[4].get_text()))
        # td6：上课时间及地点
        td6s = ''.join(tds[6].get_text().replace(u'\xa0', u' ').split())
        # “起始周数 日期 节次 地点”的模板
        pattern = re.compile(
            r'(\d\d?-\d\d?[\u4e00-\u9fa5]{3}[1-7]\(\d\d?-\d\d?[\u4e00-\u9fa5]\)[A-Z\d-]{6})')
        # 由模板匹配到的课程时间地点对组成的列表
        td6_list = pattern.findall(td6s)
        ...
        for i in td6_list:
            ...
            # 开始周数
            td6_n0 = int(pat.findall(i)[0])
            # 结束周数
            td6_n1 = int(pat.findall(i)[1])
            # 上课日期
            td6_n2 = int(pat.findall(i)[2])
            # 节次
            td6_n3 = (int(pat.findall(i)[3])+1)//2
            # 地点
            td6_n4 = re.search(r'([A-Z]\d?-\d{3,4})',i).group()
            each = [td1, td4, td6_n0, td6_n1, td6_n2, td6_n3, td6_n4]
            whole.append(each)
    with open('courses.txt', 'w+', encoding='utf-8') as f:
        for i in whole:
            tplt = '{0:{2}<20}\t{1:{2}<6}\t'
            f.write(tplt.format(i[0],i[1],chr(12288)))
            for j in i[2:6]:
                f.write(str(j)+' ')
            f.write(i[6]+'\n')
    if f:
        # time.sleep(1)
        print('\n\t分析成功\n\t已将列表化结果保存为当前目录下的“courses.txt”文件')
    return whole
...
def soc2ics(soc):
    ...
    print(separating)
    # time.sleep(1)
    print('\t写入成功！获取课表完成！\n\t请查看同级目录下的“{}-{}-{}.ics”文件'.format(name,cho_year, cho_term))
